...
//  
// 20191228-->10补数、2补数的加减法与模运算的关系(个人的一点理解):
//  这里以10补数举例: 1) 字长w=3,23与77互为二补数,求98-23的结果,用加法来实现,
//                   98+77=175,23+77=100(二补数之和),这里运用模除(modulo division)求结果
//                   用二补数之和作模, 175 mod 100 = 75与98-23=75直接运算的结果相等
//                   这是运算结果为非负数的情况
//                   2) 字长w=3,23与77互为二补数,求20-23的结果,用加法实现,
//                   20+77=97,97的10补数为3,再加上负号=-3,与20-23=-3结果一致
//             字长为w,除了全0的情况,一个数与它的10补数之和为w+1位
//       (110)
//       (101)
//       (100)        
//        011
