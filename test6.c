...
//-----------------------------------------------------------------------------------
//看完《深入》P80 2.4.3 数字实例才能确定下面这段的对错,现在一脸懵逼......还有整数运算和浮点数运算要啃,啊！我要"死"啦！！！
//指数exponent的值分三种情况,以float数据类型为例:
//  A. 规格化的值,exponent field为[0000 0001,0111 1111] --> [1,254]
//     E= e-127(bias偏置值) = [1,254]-127  ---> [-126,127]
//                          = [0000 0001,0111 1111] - [0111 1111]  
//     M(尾数的值) = 1+f(描述小数值 .f-1 f-2 ...)
//     mantissa尾数 隐含1. ,所以规格化的值无法表示0         
//
//  B. 非规格化的值,阶码字段全是0,exponent field指数字段(书本译作阶码)为[0000 0000]
//     E= 1-127(bias偏置值) = -126
//     M(尾数的值) = f(描述小数值) 
//     sign为0,exponent全为0,mantissa全为0   表示+0
//     sing为1,exponent全为0,mantissa全为0   表示-0
//
//  C  特殊值
//   C.a) exponent field[1111 1111],小数域全为0[0000 0000 0000 0000 000]
//        sign为0,+∞ 正无穷
//        sign为1,-∞ 负无穷
//        
//   C.b) exponent field[1111 1111],小数域为非0,结果值被称为"NaN"(Not a Number)
//      
//===================================================================================
//-----------------------------------------------------------------------------------
//----------这一段落待定,有可能要删除---------------
